...
st.set_page_config(page_title="Kalkulator WFH BBM", page_icon="⛽", layout="wide")

st.title("⛽ Dasbor Analitik Kebijakan WFH vs Konsumsi BBM")
st.markdown("Utak-atik parameter di sebelah kiri untuk melihat dampak riil WFH terhadap anggaran negara dan dompet rakyat.")

# --- SIDEBAR UNTUK INPUT PARAMETER ---
st.sidebar.header("⚙️ Parameter Kebijakan")
hari_wfh = st.sidebar.slider("Jumlah Hari WFH per Minggu", min_value=1, max_value=5, value=1, step=1)
kepatuhan = st.sidebar.slider("Tingkat Realisasi WFH (%)", min_value=10, max_value=100, value=100, step=5)

st.sidebar.markdown("---")
st.sidebar.subheader("Asumsi Finansial")
subsidi = st.sidebar.number_input("Besaran Subsidi per Liter (Rp)", value=1700)
harga_bbm = st.sidebar.number_input("Harga Jual Pertalite (Rp)", value=10000)

st.sidebar.markdown("<p style='text-align: center; color: #757575; margin-top: 20px;'>💡 Semua Bisa Dihitung</p>", unsafe_allow_html=True)
...
col2.metric("🏛️ Uang Negara Selamat (Subsidi)", f"Rp {hemat_subsidi_triliun_atas:.1f} T", f"Skenario {hari_wfh} hari")
col3.metric("🛍️ Uang Rakyat Selamat (Pengeluaran)", f"Rp {hemat_rakyat_triliun_atas:.1f} T", f"Skenario {hari_wfh} hari")
